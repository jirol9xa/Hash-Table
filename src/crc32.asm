...
crc32:       ; in rdi we have char * ptr
        
        push    rbx     ; saving regs
        push    rcx 
        push    rdx 
        push    rsi  

        mov     rsi, rdi     ; now we have char *ptr in rsi

        mov     edx, -1      ; Starting value  of CRC32
        mov     ecx, 32      ; Length of word
 
        xor     eax, eax
calc_crc32:
        lodsb                    ; Next symb in al
        mov     ebx, edx         ; Copy current CRC32 value in ebx
        and     ebx, 0FFh        ; ebx have 000..00bl       
        xor     bl,  al          ; XOR with current symb of string
 
        shl     ebx, 2           ; Calculating offset for dword table
        shr     edx, 8
        and     edx, 0FFFFFFh
        xor     edx, [crc_tbl + ebx] ; XOR CRC32 with table value
 
        loop    calc_crc32      ; Go to the next sybm
 
        mov     eax, edx         ; In EAX we have CRC32 value
 
        xor     eax, -1

        pop     rsi     ; Restoring regs
        pop     rdx 
        pop     rcx 
        pop     rbx 
