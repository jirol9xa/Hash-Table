...
extern "C" int comP(char *first, char *second);

const int THRESHOLD_OF_LIN = 1 << 10; //is upper lin will be qsort

#define HT                  // remove this, if u're not using list for hash-table
#define DEBUG_MODE 0
#define DEBUG(code)           \
{                             \
    if (DEBUG_MODE)           \
...
static int strCmP1(char *first, char *second);
...
int listCtor(List *list, int capacity)
{
    ...
    list->array     = (elem_t *) aligned_alloc(32, list->capacity * sizeof(elem_t));
    list->tail      = 0;
    list->head      = 0;
    list->free_head = 1;
    ...
    list->array[0].prev  = 0;
    list->array[0].next  = 0;
    list->array[0].value = _mm256_setzero_si256();
...
int listDtor(List *list)
{
    ...
    {
        DEBUG(writeLogs("!!! ERROR List is already cleaned !!!\n");)
        return -1;
    }
    listDUMP(list, __func__);
    DEBUG(ASSERT_OK(list);)
...
int listPushBack(List *list, type_t value)
{
    ...
    static int count = 0;
    //if (list->size > 1) fprintf(stderr, "cnt = %d\n", ++count);
...
//                  "value = %s | <next%d> next = %d | <prev%d> prev = %d \" ];\n", i, (list->array[i].prev == -1) ? "green" : "red", 
...
//    fclose(graph_logs);
//
//    if (system("dot -T png LOGS/GraphLogs.dot -o pic.png") == -1)
//        fprintf(stderr, "Wrong \"System\" work in listgraphdump\n");
//    if (system("eog pic.png") == -1)
//        fprintf(stderr, "Wrong \"System\" work in listgraphdump\n");
//
//}
...
int listPopBack(List *list, type_t *dest)
{
    ...
    {
        DEBUG(writeLogs("!!!! ERROR: Can't pop empty list !!!! \n"));
        return EMPTY_LIST;
    }
...
int listResize(List *list, int is_upper)
{
    ...
    if (is_upper)
    {
        ...
        //{
        //    
        //    DEBUG(writeLogs("!!! ERROR Can't allocate memory !!!\n");)
        //    fprintf(stderr, "ERROR MY MISTAKE\n");
        //    return -1;
        //}
...
int listRemove(List *list, int index, type_t *dest)
{
    ...
    if (index > list->capacity)
    {
        ...
        DEBUG(writeLogs("!!! ERROR Index is greater than capacity !!!\n");)
    ...
    if (list->array[index].prev == -1 || index == 0)
    {
        ...
        DEBUG(writeLogs("!!! ERROR Can't remove empty element !!!\n");)
    ...
    if (dest)   memcpy(dest, &(list->array[index].value), sizeof(type_t));
    list->size --;
...
//    {
//        
//        DEBUG(writeLogs("!!! ERROR Wrong list parameters in unitTest with list number --- %d!!!\n", list_num);)
//        
//        return -1;
//    }
//
...
void listDUMP(List *list, const char *func_name)
{
    ...
    DEBUG(writeLogs("In func -----> %s\n", func_name);)                                       
                                                                          
    verifyList(list);
    DEBUG(listTextDump(list);)
}


// this func only for hash-table on char *
int findElemStr(List *list, type_t elem)
{
    ...
    for (int i = list->head; i != 0; i = list->array[i].next)
    {
        if (list->status & EMPTY_LIST)  return -1;
        
        type_t value = list->array[i].value;

        __m256i temp = _mm256_sub_epi64(value, elem);
        int cmp      = _mm256_testz_si256(temp, temp);

        //printf("cmp = %d, list = %p, value = %u elem = %u\n", cmp, list, _mm256_extract_epi32(value, 0), _mm256_extract_epi32(elem, 0));
        
        if (cmp)  return i + 1;

        //if (value && !comP(value, elem))    return i;
...
static int strCmP(const char *first, const char *second)
{
    ...
    if (first[i] != second[i])  return 1;  i++;
    if (first[i] != second[i])  return 1;  i++;
    if (first[i] != second[i])  return 1;  i++;
    if (first[i] != second[i])  return 1;  i++;
    if (first[i] != second[i])  return 1;  i++;
